...
if [ $stage -le 1 ]; then
  ...
  steps/nnet/decode.sh --nj $njdec --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.10 \
     --num-threads $threads $graph_src $test $dir/decode_test || exit 1
fi
...
if [ $stage -le 2 ]; then
  dir=exp/dnn8a_${exp}_bn-feat
  # dev
  steps/nnet/make_bn_feats.sh --nj 2 --cmd "$train_cmd" $test_bn $test $dir $test_bn/log $test_bn/data || exit 1 
...
if [ $stage -le 3 ]; then
  ...
  steps/decode.sh --nj $njdec --cmd "$decode_cmd" \
     --num-threads $threads  --acwt 0.05 --beam 15.0 --lattice-beam 8.0 \
...
if [ $stage -le 7 ]; then
  ...
  # Decode test
  steps/nnet/decode.sh --nj $njdec --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.10 \
     --num-threads $threads $graph $test_bn_fmllr $dir/decode_$(basename $test_bn_fmllr) || exit 1
fi
...
if [ $stage -le 9 ]; then
  ...
  for ITER in 1 2 3 4; do
    steps/nnet/decode.sh --nj $njdec --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt $acwt \
       --num-threads $threads --nnet $dir/${ITER}.nnet \
